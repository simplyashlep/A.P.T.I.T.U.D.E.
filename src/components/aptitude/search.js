...
export async function onRequestPost(context) {
  ...
  try {
    ...
    // If the model is unavailable for ANY reason, we still hold real retrieved
    // decisions. Returning those unsummarized is far more useful than an error
    // — and for legal work, verified cases beat a paraphrase anyway.
    if (!geminiRes.ok) {
      const errBody = await geminiRes.text();
      let reason = `Model unavailable (HTTP ${geminiRes.status}).`;
      if (geminiRes.status === 429) {
        // limit: 0 means the project has no free-tier allowance at all —
        // retrying will never succeed, so don't imply that it might.
        reason = /limit:\s*0/.test(errBody)
